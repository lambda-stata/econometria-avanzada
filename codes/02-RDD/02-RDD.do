...
*** 1.6 Exportar figura
  graph export "${outputs_rdd}/fig1.pdf", replace     
      
*** 1.4 Regresiones a cada lado del cutoff
	qui: reg bexam_grade relativescore if relativescore<0 & relativescore!=0 
...
*** 1.5 Figure (scatter plot)
	twoway 	scatter bexam_grade panelb1 panelb2 relativescore 		///
			...
			ytitle(School level score) 							            ///
			xtitle(Score distance to cutoff) 					          ///
			title(Panel B: Baccalaureate grade, position(11) size(medium))      
	
*** 1.6 Exportar figura
	graph export "${outputs_rdd}/fig2.pdf", replace 
